...
/*****************************************************************************/
/*!	\file		dnp3clienttest.cs
 *	\brief 		C# Source code file, DNP3 Client/Master library test program
 *
...
namespace dnp3clienttest
{
    class Program
    {
        ...
        ******************************************************************************/
        static short cbClientStatus(ushort u16ObjectId, ref dnp3types.sDNP3DataAttributeID psDAID, ref dnp3types.eServerConnectionStatus peSat, ref short ptErrorValue)
	    {
            Console.WriteLine("\r\n cbClientstatus called");

            if (psDAID.eCommMode == dnp3types.eCommunicationMode.COMM_SERIAL)
            {
        ...
        ******************************************************************************/
        static short cbPollStatus(ushort u16ObjectId, ref dnp3types.sDNP3DataAttributeID ptUpdateID, dnp3types.eWriteFunctionID eFunctionID, ref short ptErrorValue)
        {
            //u16ObjectId *= u16ObjectId;
            Console.WriteLine("\r\n Poll status callback called , Object ID " + u16ObjectId);


            if (ptUpdateID.eCommMode == dnp3types.eCommunicationMode.COMM_SERIAL)
            {
            ...
            switch (eFunctionID)
            {
                case dnp3types.eWriteFunctionID.READCLASS0123:
                    Console.Write("\t integrity poll class 0123 compleated");
                    break;

                default:
                    Console.Write("\t Invalid function id");
                    break;
            }
        ...
        * Update IIN callback
        ******************************************************************************/
        static short cbUpdateIIN( ushort u16ObjectId, ref dnp3types.sDNP3DataAttributeID ptUpdateID, byte u8IIN1, byte u8IIN2, ref short ptErrorValue)
        {
            //u16ObjectId *= u16ObjectId;
            ...
            if ((u8IIN1 & (byte)dnp3types.eIINFirstByteBitsFlag.BROADCAST) == (byte)(dnp3types.eIINFirstByteBitsFlag.BROADCAST))
            {
                Console.WriteLine("BROADCAST");
            }

            if ((u8IIN1 & (byte)dnp3types.eIINFirstByteBitsFlag.CLASS_1_EVENTS) == (byte)dnp3types.eIINFirstByteBitsFlag.CLASS_1_EVENTS)
            {
                Console.WriteLine("CLASS_1_EVENTS");
            }

            if ((u8IIN1 & (byte)dnp3types.eIINFirstByteBitsFlag.CLASS_2_EVENTS) == (byte)dnp3types.eIINFirstByteBitsFlag.CLASS_2_EVENTS)
            {
                Console.WriteLine("CLASS_2_EVENTS");
            }

            if ((u8IIN1 & (byte)dnp3types.eIINFirstByteBitsFlag.CLASS_3_EVENTS) == (byte)dnp3types.eIINFirstByteBitsFlag.CLASS_3_EVENTS)
            {
                Console.WriteLine("CLASS_3_EVENTS");
            }

            if ((u8IIN1 & (byte)dnp3types.eIINFirstByteBitsFlag.NEED_TIME) == (byte)dnp3types.eIINFirstByteBitsFlag.NEED_TIME)
            {
                Console.WriteLine("NEED_TIME");
            }

            if ((u8IIN1 & (byte)dnp3types.eIINFirstByteBitsFlag.LOCAL_CONTROL) == (byte)dnp3types.eIINFirstByteBitsFlag.LOCAL_CONTROL)
            {
                Console.WriteLine("LOCAL_CONTROL");
            }

            if ((u8IIN1 & (byte)dnp3types.eIINFirstByteBitsFlag.DEVICE_TROUBLE) == (byte)dnp3types.eIINFirstByteBitsFlag.DEVICE_TROUBLE)
            {
                Console.WriteLine("DEVICE_TROUBLE");
            }

            if ((u8IIN1 & (byte)dnp3types.eIINFirstByteBitsFlag.DEVICE_RESTART) == (byte)dnp3types.eIINFirstByteBitsFlag.DEVICE_RESTART)
            {
                Console.WriteLine("DEVICE_RESTART");
            }

            if ((u8IIN2 & (byte)dnp3types.eIINSecondByteBitsFlag.NO_FUNC_CODE_SUPPORT) == (byte)dnp3types.eIINSecondByteBitsFlag.NO_FUNC_CODE_SUPPORT)
            {
                Console.WriteLine("NO_FUNC_CODE_SUPPORT");
            }

            if ((u8IIN2 & (byte)dnp3types.eIINSecondByteBitsFlag.OBJECT_UNKNOWN) == (byte)dnp3types.eIINSecondByteBitsFlag.OBJECT_UNKNOWN)
            {
                Console.WriteLine("OBJECT_UNKNOWN ");
            }

            if ((u8IIN2 & (byte)dnp3types.eIINSecondByteBitsFlag.PARAMETER_ERROR) == (byte)dnp3types.eIINSecondByteBitsFlag.PARAMETER_ERROR)
            {
                Console.WriteLine("PARAMETER_ERROR ");
            }

            if ((u8IIN2 & (byte)dnp3types.eIINSecondByteBitsFlag.EVENT_BUFFER_OVERFLOW) == (byte)dnp3types.eIINSecondByteBitsFlag.EVENT_BUFFER_OVERFLOW)
            {
                Console.WriteLine("EVENT_BUFFER_OVERFLOW");
            }

            if ((u8IIN2 & (byte)dnp3types.eIINSecondByteBitsFlag.ALREADY_EXECUTING) == (byte)dnp3types.eIINSecondByteBitsFlag.ALREADY_EXECUTING)
            {
                Console.WriteLine("ALREADY_EXECUTING ");
            }

            if ((u8IIN2 & (byte)dnp3types.eIINSecondByteBitsFlag.CONFIG_CORRUPT) == (byte)dnp3types.eIINSecondByteBitsFlag.CONFIG_CORRUPT)
            {
                Console.WriteLine("CONFIG_CORRUPT ");
            }

            if ((u8IIN2 & (byte)dnp3types.eIINSecondByteBitsFlag.RESERVED_2) == (byte)dnp3types.eIINSecondByteBitsFlag.RESERVED_2)
            {
                Console.WriteLine("RESERVED_2 ");
            }

            if ((u8IIN2 & (byte)dnp3types.eIINSecondByteBitsFlag.RESERVED_1) == (byte)dnp3types.eIINSecondByteBitsFlag.RESERVED_1)
            {
        ...
        * Update callback
        ******************************************************************************/
        static short cbUpdate(ushort u16ObjectId, ref dnp3types.sDNP3DataAttributeID ptUpdateID, ref dnp3types.sDNP3DataAttributeData ptUpdateValue, ref dnp3types.sDNP3UpdateParameters ptUpdateParams, ref short ptErrorValue)
        {
            Console.WriteLine("\r\n cbUpdate called");
            ...
            {
                case dnp3types.eDNP3GroupID.BINARY_INPUT:  /*!< Binary Input (DNP3Group 1) */
                case dnp3types.eDNP3GroupID.DOUBLE_INPUT: /*!< Double-bit Binary Input Input (DNP3Group 3) */
                case dnp3types.eDNP3GroupID.BINARY_OUTPUT:/*!< Binary Output (DNP3Group 10) */
                    Console.Write("\t Data : {0:D}", System.Runtime.InteropServices.Marshal.ReadByte(ptUpdateValue.pvData));
                    break;


                case dnp3types.eDNP3GroupID.COUNTER_INPUT: /*!< Counter Input (DNP3Group 20) */
                case dnp3types.eDNP3GroupID.FRCOUNTER_INPUT: /*!< Frozen Counter Input (DNP3Group 21) */
                    Console.Write("\t Data : {0:D}", System.Runtime.InteropServices.Marshal.ReadInt32(ptUpdateValue.pvData));
                    break;


                case dnp3types.eDNP3GroupID.ANALOG_INPUT: /*!< Analog Input (DNP3Group 30) */
                case dnp3types.eDNP3GroupID.FRANALOG_INPUT: /*!< Frozen Analog Input (DNP3Group 30) */
                case dnp3types.eDNP3GroupID.ANALOG_OUTPUTS: /*!< Analog output (DNP3Group 40) */

                    if (ptUpdateValue.eDataType == tgtcommon.eDataTypes.FLOAT32_DATA)
                    {
                    ...
                    else if (ptUpdateValue.eDataType == tgtcommon.eDataTypes.SIGNED_DWORD_DATA)
                    {
                        Console.Write("\t Data : {0:D}", System.Runtime.InteropServices.Marshal.ReadInt32(ptUpdateValue.pvData));
                    }
                    else
                ...
                case dnp3types.eDNP3GroupID.OCTECT_STRING:
                case dnp3types.eDNP3GroupID.VIRTUAL_TERMINAL_OUTPUT:
                        Console.Write("\t Data : " + System.Runtime.InteropServices.Marshal.PtrToStringUTF8(ptUpdateValue.pvData));
            ...
            if ((ptUpdateValue.tQuality & (ushort)dnp3types.eDNP3QualityFlags.ONLINE) == (ushort)dnp3types.eDNP3QualityFlags.ONLINE)
            {
                Console.Write("\t ONLINE");
            }
            if ((ptUpdateValue.tQuality & (ushort)dnp3types.eDNP3QualityFlags.RESTART) == (ushort)dnp3types.eDNP3QualityFlags.RESTART)
            {
                Console.Write("\t RESTART");
            }

            if ((ptUpdateValue.tQuality & (ushort)dnp3types.eDNP3QualityFlags.COMM_LOST) == (ushort)dnp3types.eDNP3QualityFlags.COMM_LOST)
            {
                Console.Write("\t COMM_LOST");
            }

            if ((ptUpdateValue.tQuality & (ushort)dnp3types.eDNP3QualityFlags.REMOTE_FORCED) == (ushort)dnp3types.eDNP3QualityFlags.REMOTE_FORCED)
            {
                Console.Write("\t REMOTE_FORCED");
            }

            if ((ptUpdateValue.tQuality & (ushort)dnp3types.eDNP3QualityFlags.LOCAL_FORCED) == (ushort)dnp3types.eDNP3QualityFlags.LOCAL_FORCED)
            {
                Console.Write("\t LOCAL_FORCED");
            }

            if ((ptUpdateValue.tQuality & (ushort)dnp3types.eDNP3QualityFlags.CHATTER_FILTER) == (ushort)dnp3types.eDNP3QualityFlags.CHATTER_FILTER)
            {
                Console.Write("\t CHATTER_FILTER");
            }

            if ((ptUpdateValue.tQuality & (ushort)dnp3types.eDNP3QualityFlags.ROLLOVER) == (ushort)dnp3types.eDNP3QualityFlags.ROLLOVER)
            {
                Console.Write("\t ROLLOVER");
            }

            if ((ptUpdateValue.tQuality & (ushort)dnp3types.eDNP3QualityFlags.OVER_RANGE) == (ushort)dnp3types.eDNP3QualityFlags.OVER_RANGE)
            {
                Console.Write("\t OVER_RANGE");
            }

            if ((ptUpdateValue.tQuality & (ushort)dnp3types.eDNP3QualityFlags.DISCONTINUITY) == (ushort)dnp3types.eDNP3QualityFlags.DISCONTINUITY)
            {
                Console.Write("\t DISCONTINUITY");
            }

            if ((ptUpdateValue.tQuality & (ushort)dnp3types.eDNP3QualityFlags.REFERENCE_ERR) == (ushort)dnp3types.eDNP3QualityFlags.REFERENCE_ERR)
            {
            ...
            if(ptUpdateParams.eUpCause == dnp3types.eUpdateCause.STATIC_DATA)
                Console.Write(" Cause: Static Data");
            else if (ptUpdateParams.eUpCause == dnp3types.eUpdateCause.POLLED_EVENT)
                Console.Write(" Cause: Polled Event");
            else if (ptUpdateParams.eUpCause == dnp3types.eUpdateCause.UNSOLICITED_EVENT)
        ...
        ******************************************************************************/
        static short cbDeviceAttribute(ushort u16ObjectId, ref dnp3types.sDNP3DataAttributeID psDAID, ref dnp3types.sDNP3DeviceAttributeData psDeviceAttrValue, ref short ptErrorValue)
        {
        ...
        ******************************************************************************/        
        static void Main(string[] args)
        {
            ...
            dnp3types.sDNP3Parameters sParameters;    // DNP3 client object callback paramters 
            dnp3types.sDNP3DataAttributeID sDAID;
            dnp3types.sDNP3DataAttributeData sNewValue;
            dnp3types.sDNP3CommandParameters sParams;                    // command data- date and time structute  

            System.Console.WriteLine("\n\r\n \t\t**** DNP3 Protocol Client Master Library Test ****");
            try
            {
            ...
            sParameters.eAppFlag = tgtcommon.eApplicationFlag.APP_CLIENT;			            // This is a DNP3 client   
            sParameters.ptReadCallback = null;					                            // Read Callback
            sParameters.ptWriteCallback = null;				// Write Callback           
            sParameters.ptUpdateCallback = new dnp3types.DNP3UpdateCallback(cbUpdate);					                        // Update Callback
            ...
            sParameters.ptClientStatusCallback = new dnp3types.DNP3ClientStatusCallback(cbClientStatus);  // client connection status callback
            sParameters.ptDeviceAttrCallback = new dnp3types.DNP3DeviceAttributeCallback(cbDeviceAttribute); // Device attribute callback
            sParameters.ptUpdateIINCallback = new dnp3types.DNP3UpdateIINCallback(cbUpdateIIN);  // IIN internal indication update callback
            sParameters.ptClientPollStatusCallback = new dnp3types.DNP3ClientPollStatusCallback(cbPollStatus);  // poll status callback
             ...
             do
            {
                ...
                DNP3Clienthandle = dnp3api.DNP3Create(ref sParameters, ref iErrorCode, ref ptErrorValue);
...
#if VIEW_TRAFFIC
                sDNP3Config.sDNP3ClientSet.sDebug.u32DebugOptions = (uint)(tgtcommon.eDebugOptionsFlag.DEBUG_OPTION_TX | tgtcommon.eDebugOptionsFlag.DEBUG_OPTION_RX);
#else
	   sDNP3Config.sDNP3ClientSet.sDebug.u32DebugOptions = 0;
                ...
                dnp3types.sClientObject[] psClientObjects = new dnp3types.sClientObject[sDNP3Config.sDNP3ClientSet.u16NoofClient];
    ...
    // tcp communication settings
     ...
     psClientObjects[0].eCommMode = dnp3types.eCommunicationMode.TCP_IP_MODE;
   ...
   psClientObjects[0].psDNP3Objects = System.IntPtr.Zero;
     ...
     System.Runtime.InteropServices.Marshal.StructureToPtr(psClientObjects[0], tmp1, true);
                ...
                // Load configuration
    iErrorCode = dnp3api.DNP3LoadConfiguration(DNP3Clienthandle, ref sDNP3Config, ref ptErrorValue);
    if(iErrorCode != 0)
    {
    ...
    // Start server
    iErrorCode = dnp3api.DNP3Start(DNP3Clienthandle, ref ptErrorValue);
    if (iErrorCode != 0)
    {
    ...
    sDAID = new dnp3types.sDNP3DataAttributeID();
    sNewValue = new dnp3types.sDNP3DataAttributeData();
    sParams = new dnp3types.sDNP3CommandParameters();                    // command data- date and time structute  

    //command parameters
#if  CLIENT_TCP_COMMUNICATION
    sDAID.eCommMode    =   dnp3types.eCommunicationMode.TCP_IP_MODE;
    ...
    sDAID.eGroupID = dnp3types.eDNP3GroupID.ANALOG_OUTPUTS;
    sDAID.u16SlaveAddress = 1;
    sDAID.u16IndexNumber = 0;
    sDAID.pvUserData = IntPtr.Zero;

    sNewValue.eDataSize = tgttypes.eDataSizes.FLOAT32_SIZE;
    sNewValue.eDataType = tgtcommon.eDataTypes.FLOAT32_DATA;
    sNewValue.tQuality = (ushort)dnp3types.eDNP3QualityFlags.GOOD;
    sParams.eCommandVariation = dnp3types.eCommandObjectVariation.ANALOG_OUTPUT_BLOCK_FLOAT32;
    ...
    sNewValue.pvData = System.Runtime.InteropServices.Marshal.AllocHGlobal((int)tgttypes.eDataSizes.FLOAT32_SIZE);
    sParams.u8Count    =   1;
                ...
                while (true)
                {
                    ...
                    else
                    {
                        ...
                        iErrorCode = dnp3api.DNP3DirectOperate(DNP3Clienthandle, ref sDAID, ref sNewValue, ref sParams, ref ptErrorValue);
                        if (iErrorCode != 0)
                        {
                            Console.WriteLine("dnp3 Library API Function - DNP3DirectOperate() Analog Output Command failed: {0:D} {1:D}", iErrorCode, ptErrorValue);
                            System.Console.WriteLine("iErrorCode {0:D}: {1}", iErrorCode, errorcodestring(iErrorCode));
                            System.Console.WriteLine("iErrorValue {0:D}: {1}", ptErrorValue, errorvaluestring(ptErrorValue));
                ...
                // Stop server
                iErrorCode = dnp3api.DNP3Stop(DNP3Clienthandle, ref ptErrorValue);
                if (iErrorCode != 0)
                {
                ...
                // Free server
                iErrorCode = dnp3api.DNP3Free(DNP3Clienthandle, ref ptErrorValue);
                if (iErrorCode != 0)
                {
